Shader "Custom/SinEffectVertical"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float AmpSmoothening = (cos(i.uv.x * TAU) + 1.0) * 0.5; // cos = (cos(x) + 1) / 2
                AmpSmoothening *= AmpSmoothening;
                AmpSmoothening *= AmpSmoothening;
                AmpSmoothening = 1.0 - AmpSmoothening * AmpSmoothening; // 1 - cos^8

                return tex2D(_MainTex, i.uv + float2(sin((i.uv.y - _Phase) * _Frequency * TAU) * _Amplitude * AmpSmoothening, 0.0));
            }
